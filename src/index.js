...
// try catch для стратицы описания товара
  ...
  if (card) {
    ...
    getGoodsItem(id)
      .then((item) => {
        renderItem(item);
        preload.remove();
        // для составления списка рекомендуемых товаров берем категорию продукта
      ...
      .then((recommendedGoods) => {
        renderRecommendedItems(recommendedGoods.goods, id);
      });
  }
} catch (error) {
  console.warn(error);
